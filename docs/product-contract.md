...
`assignments.agents.<id>` 是 route 下唯一的 managed opt-in（稳定 agent id；CLI `--for` 写入该覆盖）。**没有覆盖的 Bot 回官方 Host session。** `assignments.main` 可选，不是未覆盖 Bot 的回退。`activate --mode route` 允许 agents-only（`main` 可为 null）；已出现的赋值须为 `stub/echo` 或 openai*（http(s) endpoint + 非空 `apiKeyRef`，含 `env:` / `file:` / 适配得到的 `pi-provider:`）；其它赋值 fail-closed。`models use` / `activate --mode route` 必须披露：provider/endpoint、数据类型、下个 turn 生效、改的是默认还是某一 Bot。明确 `--default` 的 `use` 写 `main`，不把其它 Bot 拉进 modeld；`--for` 与 `--default` 必须且只能选一个。**目标：route期间`models reset --for <bot>`必须支持仅该Bot下一TURN回原生official，不全局deactivate、不改harness。** 当前源码已实现该 targeted reset，且撤销 override 不要求当前 ownership 仍能准许 managed 执行；本轮结构化 assignment 保留这一安全退出。合法未opt-in和用户明确official，与损坏/不可读配置导致decline严格区分；配置saved与当前TURN captured分开投影。

**同通道推理设置（schema v2 / wire v7）**：assignment 为 `{modelId, reasoning?:{effort}}`，不物化派生 catalog。`models use --effort` 仅接受该 endpoint/API/wire model 明确声明的 capability 子集；未知能力、不支持的档位和未知字段拒绝，不换通道、不降档。省略 effort 或 `default` 完整清除 override，`none` 需明确支持。有效选择与 capability 进入 selectionRevision，当前 TURN/冷恢复使用原绑定，新 TURN 才使用修改。`models show --for` 为 configured-next-turn 纯投影，UUID 查询无需 Provider 或 Gateway。

schema v1 只读归一化为 v2；明确授权的模型保存及 `models migrate --confirm` 写 v2。升级前须保护原配置并成套切换 CLI/preload/Host/modeld；旧版本拒绝新 schema，新 wire 对旧 peer 只读诊断而不准执行。配置根不变，既不自动 migrate 路径，也不复制 credential。`reasoningTokens` 是 completionTokens 的可选子集，不重复计费/累计、不把缺值当零。终态区分 requested/emitted/providerReported（当前 unknown），标题 `e=` 只代表所请求档位。SDK warnings 有界且去私密化，HTTP 前验证最终档位与冻结值一致；任何 model ID 伪装、通道替换或冲突均不被容忍。详见[决策](decisions/2026-09-17-model-reasoning-policy.md)。
