...
The current PatchProfile requires `create-session` and `agent-id` (`host.getConversationId()` plus `invocationId: inferenceRequestId`). Optional slices include `compact-register`, `activity-bridge`, `memory-purpose`, `episode-purpose`, harness always-emit, and harness persist; authoring accepts only unique approved ids. Missing optional hooks are inert. The D2-approved [E07 purpose seam](maintainers/e07-path-b-host-admission.md) labels only the two real memory/episode factories and binds them to an actual completed STEP in the same managed session, never to prompt text or executor counts. Route seam sends a Bot to modeld only when `assignments.agents[agentId]` is set to an admitted managed model; missing override is official passthrough. `assignments.main` is not a session fallback. T11: pre-dispatch local failures (missing models/resolve/TURN/socket) return `originalSession`; after wrap or provider dispatch the Host sees a correlated managed error (`agentId`, STEP, `stage=admit|provider|normalize`) and never a silent official replay. Debug canary is grokbox test0 `00000000-0000-4000-8000-000000000114`. grokbox test1 `00000000-0000-4000-8000-000000000113` stays official until opted in.
...
- `runtime start --mode observe|identity|route`: unified ensure facade. Probes `modeld.sock` and, if down, starts the stub server in-process on the same listen path as `modeld run` (no `wait()`, no second daemon). Writes desired with `activate` semantics, runs one `runWatchdogTick` for identity/route only, then prints `projectLiveStatus`. Default path never re-adopts or canaries and does not merge watchdog into `daemon serve`.
...
- `runtime modeld run`: Unix framing/client lifecycle (`modeld-ipc.ts`) composes the **one** admission/pinning kernel (`modeld.ts`) with canonical file ports (`modeld-store.ts`). Per-Bot config and fingerprints resolve only there, after compiled Host binding admission and before an independently rechecked driver effect. Current default driver is credential/network-hard-off `stub/echo`; fake tests replace dependencies, not admission. A+S1 (`modeld-as1.ts`) is an optional `ModeldDriver` that buffers an injected generate-port iterable into `complete()`. T4b `createOpenAiModeldDriver` (`modeld-openai.ts`) is the AI SDK OpenAI Chat Completions / Responses adapter behind that port (`endpoint` = baseURL). It is not the CLI default; `ai` / `@ai-sdk/openai` live only on `@grokbox/box-runtime` and must not be imported from preload/seam/session/hook. Service-incarnation fencing, bounded TTL/tombstones/capacity and abort-on-disconnect/stop do not imply durable resume or provider streaming. Timeout is a visible managed failure, not official fallback. See `box-runtime.md` §5 for current bounds.
