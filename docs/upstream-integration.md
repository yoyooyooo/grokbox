...
### Native checkpoint closure and readback qualification

The separately pinned Host/worker pair in `test/native-continuity-code.ts` provides protobuf reference metadata and an original AgentStore checkpoint writer/reader. Its ordinary GC/export traversal deliberately omits summary-archive and historical conversation-root edges. A recovery reader must not call that reduced graph complete: `host/native-checkpoint.ts` follows all qualified reference metadata, including nested maps/oneofs and cycles, preserves exact payload bytes, and enforces part/byte/edge bounds. Unknown wire fields, missing references, invalid declared JSON/UTF-8, conflicting types or absent root-reference metadata refuse this native candidate. Best-effort semantic reconstruction remains a separate path, not silent loss of references from a supposedly complete native snapshot.

The original AgentStore can overwrite a fixed root slot without changing its ID. It writes the blob before updating metadata and the in-memory structure, so a metadata failure may follow an actual root write. Its reset method catches corrupt/missing state and can silently install an empty structure. Strict readback therefore checks root bytes, decoded in-memory state and the complete qualified reference graph against the immutable material; a successful reset or matching pointer alone is insufficient.

Normal native session opening may ensure profile/settings, repair hidden transcript entries, recover missing roots and run storage maintenance. It is not a read-only capture primitive. The capture adapter requires an explicitly acquired native read boundary and bounded producer; matching before/after reads do not replace exclusion from checkpoint writers or GC. No production acquire binding, persistent application marker or activation permission is created by this adapter.

The opt-in `native-checkpoint-qualification.test.ts` and `native-checkpoint-process.test.ts` execute selected original codecs/AgentStore with owned files/metadata and the real CONT vault, including a fresh Node reader after removing the owned source. They do not execute the original Agent loop, native worker process/DB transaction, live preparation fence, Memory/display-history import or actual provider request. Ordinary tests use a public reflection fixture; the older whole-Host pin is unchanged. [CONT-00](tickets/CONT-00-native-clone-feasibility.md) and the [fixed report](reports/2026-09-18-continuity-native-checkpoint.md) own the narrow qualification and remaining work.
