...
- [Transcript harness: box vs temporal](maintainers/transcript-harness-box-vs-server.md): managed intercept needs `harness=box`; `agents update` cannot persist it; stock dual ledger is grok-bot `private interoperability notes (not distributed)`. Not a Host patch.
