...
- [单盒状态塑造、替身与交接 Spec S13](roadmap/box-runtime-impl-spec.md#ownership-continuity)：北极星是尽力保真恢复后新Bot边接活、程序/旧Bot边交接，持续监控旧入站直至条件式退役；[CONT-00–11全程规划](tickets/README.md#ownership-continuity)包含分档保护、默认暂停Routine、duplicate/clone/replace、唯一当前context和受管指令spawn。没有多session/跨机器；[CONT-02/11首个真实存储消费者](reports/2026-09-18-continuity-recovery-store.md)已通过限定的材料发布、保护、unknown去重与Node进程验证，[CONT-07当前状态协调](reports/2026-09-18-continuity-current-state.md)已补捕获/初始化/对账和Node故障验证；[原生checkpoint格式与严格读回](reports/2026-09-18-continuity-native-checkpoint.md)进一步使用实际Host/worker代码完成限定资格；[当前状态操作](maintainers/current-state-control.md)现已提供原生worker事务、主Host准备屏障/应用凭据、有限RPC和手动CLI，仍需固定profile与配置成套现场采用，完整产品链未交付；现场只看 [LIVE-OWNERSHIP-CONTINUITY及关联维度](tickets/LIVE-integration-validation.md#live-ownership-continuity)。
- [2026-09-19 连续性候选集成证据](reports/2026-09-19-continuity-lifecycle-integration.md)：有限clone/replace/spawn、外部空闲reset/recover、保护与交接已接线的测试范围；明确附件、自重置、外部任务、临时清理和删除边界仍未完成，不代表现场采用或完整北极星关闭。
- [官方式 duplicate 操作](maintainers/native-agent-duplicate.md)：只读计划、明确确认、持久新身份回执、未知结果不重建和独立归属读回；[CONT-06](tickets/CONT-06-native-duplicate-cli.md)为实现/资格入口，真实账户与App效果只看[LIVE-NATIVE-DUPLICATE](tickets/LIVE-integration-validation.md#live-native-duplicate)。它不是完整状态clone，未切换现役。
