...
| <a id="live-network-boundary"></a>**LIVE-NETWORK-BOUNDARY**<br>G1/G3 | [ ] `partial`；`not-run` | ①默认/本地init不调用Tailscale、不自动选远端；②外部自管HTTPS入口在无Tailscale CLI客户端上doctor准确且不读Serve；③认证/能力/监听器失败拒绝mutation，健康recover无SSH即no-op；④受控已安装daemon仅SSH ensure，wake只在provider确认休眠后；⑤仅被明确选择的旧部署用`--legacy-tailnet`验证精确映射/漂移拒绝，无第三方配置变更 | 来源与实施差额：[网络合同](../product-contract.md#2-默认入口与连接) · [NET-01兼容与步骤](NET-01-box-local-network-boundary.md#compatibility-and-operator-migration) |
