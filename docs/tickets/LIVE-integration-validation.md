...
| <a id="live-network-boundary"></a>**LIVE-NETWORK-BOUNDARY**<br>G1/G3 | `partial`；`awaiting-integration`；基于4181e5e；[NET-01离线证明](NET-01-box-local-network-boundary.md#offline-proof)不算现场通过 | ①默认/本地init不调用Tailscale、不自动选远端；②外部自管HTTPS入口在无Tailscale CLI客户端上doctor准确且不读Serve；③认证/能力/监听器失败拒绝mutation，健康recover无SSH即no-op；④受控已安装daemon仅SSH ensure，wake只在provider确认休眠后；⑤仅被明确选择的旧部署用`--legacy-tailnet`验证精确映射/漂移拒绝，无第三方配置变更 | DEP/REVIEW：先将NET-01合入固定V2候选并完成review；普通连接/应用健康属G1，真实wake与旧mapping变更属另行授权G3，未选不阻Box-local功能；外部DNS/TLS/IP/IPv6不能凭mock签绿；[网络合同](../product-contract.md#22-网络与旧部署兼容边界) · [NET-01兼容与步骤](NET-01-box-local-network-boundary.md#compatibility-and-operator-migration) |
