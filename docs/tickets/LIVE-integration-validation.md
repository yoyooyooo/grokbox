...
| <a id="live-obs-storage"></a>**LIVE-OBS-STORAGE**<br>长期容量与物理空间回收 | **本轮只读取证，未切换现役。** schema4候选初始提交2e876bc（分支`docs/incident-bot-observability-plan`）补journal配置采用、缩额/配置删除保护、collector内维护及目录锁v2真实子进程硬崩恢复；专项90 pass、全仓2368 pass/15 skip/0 fail。候选Node只读control legacy journal为80221字节，明确monitor未初始化、本次runRoot未配置；随后核对现役仍schema3。 | 匹配Host/preload/modeld与schema4成套采用、实际维护周期/重启/物理稳态、SQLite辅助文件及全安装预留、其他owner与安全退役。旧PID-only锁和撕裂文件无自动清理资格；离线维护不证明安装自启。 | `blocked`：[T51](T51-ops-capability-presets.md)/[OBS-04](OBS-04-bounded-observation-storage.md)/[OBS-06](OBS-06-integration-and-soak-proof.md)候选尚未合入，独立review无有效回执。最后文档/隐私联合检查被工具拦截，未计通过；此前源码专项扫描通过。与[CONFIG-CUTOVER](#live-config-cutover)固定原制品退路后成套集成采用，不只更新源码shim。候选固定回执：`docs/reports/2026-09-18-journal-policy-and-lock-recovery.md`；已合入历史见[分段回执](../reports/2026-09-18-structured-journal-rotation.md)。 |
