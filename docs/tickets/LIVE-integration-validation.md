...
- **规划窗口：RC-E2E-20260919，尚未执行。** E2E前置实现与离线组合已收口；当前仍未迁移现役配置、切 Host/modeld/daemon、创建测试 Bot、发送生产 Webhook或发布。
- **源码范围以开窗前冻结的v2为准。** 观测→固定现场→配对/发送、daemon collector、诊断联合接纳、execution/context/provision安全收缩及精确服务注册均有离线实现。服务注册要求可达且linger开启的systemd用户管理器；不等于当前Box已具备该环境或整个OS已受配额约束。[前置收口证据](../reports/2026-09-19-pre-e2e-closeout.md)保存限定证明，不代替实际加载。
...
- **当前放行门**：前置固定候选全仓已按无遗漏文件组完成，旧“92文件缺失”和shim超时不再是本候选阻断；独立审查仍无报告，目标管理器/原生版本与退路须在W0核验。REVIEW与ENV按对应场景处理，不把超时记作通过，也不反复重开已完成实现。
...
|---|---|---|---|
| <a id="live-release-candidate"></a>**LIVE-RELEASE-CANDIDATE**<br>G0 | `integrated`；`not-run`；前置实现和完整离线回归已收口，未开始新窗口 | ①固定干净v2源码/锁/Node包/CLI/preload/profile/worker原生版本；②完整用例清单无遗漏，独立review有结论；③登记模型与对象预算、原状态快照和可执行旧制品；④区分代码合入、部署、发布 | REVIEW/ENV：独立审查尚无报告；先按[前置证据](../reports/2026-09-19-pre-e2e-closeout.md#测试独立审查与证据层级)及[窗口手册](../maintainers/live-end-to-end.md#window-record)冻结候选/核验真实环境，不以离线通过签实际采用 |
...
| <a id="live-network-boundary"></a>**LIVE-NETWORK-BOUNDARY**<br>G1/G3 | `integrated`；`not-run`；[固定V2合入与复验回执](../reports/2026-09-19-network-boundary-v2-integration.md)不算现场通过 | ①默认/本地init不调用Tailscale、不自动选远端；②外部自管HTTPS入口在无Tailscale CLI客户端上doctor准确且不读Serve；③认证/能力/监听器失败拒绝mutation，健康recover无SSH即no-op；④受控已安装daemon仅SSH ensure，wake只在provider确认休眠后；⑤仅被明确选择的旧部署用`--legacy-tailnet`验证精确映射/漂移拒绝，无第三方配置变更 | REVIEW/ENV/AUTH：NET-01独立代码审查尚无结论，现场须固定受控endpoint/SSH环境与窗口授权；普通连接/应用健康属G1，真实wake与旧mapping变更属另行授权G3，未选不阻Box-local功能；外部DNS/TLS/IP/IPv6不能凭mock签绿；[网络合同](../product-contract.md#2-默认入口与连接) · [NET-01兼容与步骤](NET-01-box-local-network-boundary.md#compatibility-and-operator-migration) |
...
| <a id="live-runtime-persistence"></a>**LIVE-RUNTIME-PERSISTENCE**<br>G2 | `integrated`；`blocked` ENV；精确服务注册/退场已实现，本机用户管理器未可达 | ①受支持service owner安装幂等；②父shell/网页退出服务仍活；③正常退出/重新启动单实例；④Box重启/环境重建后配置/凭据与加载恢复；⑤on/off不暗切Host或越权 | ENV/REVIEW：重查管理器/linger和发行包，按[服务注册](../maintainers/runtime-service-registration.md)执行；不以nohup替代，平台Reset另行授权；[T40](T40-persistent-release-and-rollback.md) |
| <a id="live-monitor-persistence"></a>**LIVE-MONITOR-PERSISTENCE**<br>G2 | `integrated/partial`；`not-run`；daemon collector/精确socket恢复已合入v2，未现役采用 | ①canonical runRoot/durableRoot和明确目标集合；②collector随正确宿主存续且重启接续cursor；③慢RPC不挡本地故障，scope/缺源显示gap；④ack/snooze/固定现场持久；⑤无UI也持续生产新work | REVIEW/ENV：服务注册实现已提供；本机管理器与实际采用待核验，不能用detach代签。见[服务内证明](../reports/2026-09-19-pre-e2e-observation.md#collector-lifetime)；[T41](T41-continuous-observation-and-alerting.md) · [T44](T44-host-ops-continuous-sensing.md) · [T50](T50-template-ops-release-proof.md) |
| <a id="live-obs-storage"></a>**LIVE-OBS-STORAGE**<br>G2 | `integrated/partial`；`not-run`；诊断writer联合接纳/24轮soak已合入v2，未现场验证 | ①真实文件系统滚动/缩额/闲置GC计量含索引/辅助/暂存；②通知off仍GC；③至少跨三个维护周期及窗口内受控到期，查询无隐式GC；④24小时扩展观察另留窗口；⑤未覆盖owner与旧writer明确，不签全安装或OS quota | REVIEW/ENV/CODE：固定制品与实际采用待验，其他安全/制品owner仍归来源票；不在生产填满盘或删历史。见[诊断接纳证据](../reports/2026-09-19-diagnostic-admission.md#固定执行证据)；[OBS-04](OBS-04-bounded-observation-storage.md) |
| <a id="live-obs-safe-retirement"></a>**LIVE-OBS-SAFE-RETIREMENT**<br>G2/G3 | `integrated`；`not-run`；各owner安全收缩与实际准入拒重入已有离线证据 | ①引用闭包/最后可靠点不被诊断GC删除；②旧STEP/commit_unknown/notify unknown/provision在GC及新进程后拒重复副作用；③明细退役保留精确保护，未结前缀不饿死其余GC；④验证支持的通知DB恢复与服务epoch边界，容量满不清账本 | REVIEW/ENV：按[owner边界](../reports/2026-09-19-pre-e2e-closeout.md#安全退役最小保护由执行入口消费)逐项现场核验，不扩成任意整机镜像回滚保证；[OBS-05](OBS-05-safe-state-retirement.md) · [CONT存储](CONT-02-continuity-snapshots.md) |
...
| `runtime monitor init`, `runtime monitor install`, `runtime monitor service`, `runtime monitor run`, `runtime monitor snapshot`, `runtime monitor events`, `runtime monitor incidents`, `runtime monitor ack`, `runtime monitor snooze` | [LIVE-MONITOR-PERSISTENCE](#live-monitor-persistence) | install显式配置／初始化但不启动；service只读；持久宿主与Box自启分开验 |
