...
|---|---|---|---|
| <a id="live-ops-routines"></a>**LIVE-OPS-ROUTINES**<br>原生 Routine / Payload / 模板隔离 | **本轮未操作真实配对或发送Webhook。** 候选补Box-local配对preview/bind/状态/撤销、固定私有capsule与未知重入保护；prepared始终deliveryAuthorized=false。最新v2含HCR组合上专项106 pass、HCR52 pass、全仓2568 pass/19 skip/0 fail，J1接口保持。合成HTTP/真实SQLite/强杀/packed Node不算原生配对或Bot收到。 | 当前原生HTTP/凭据资格、接收者模型/提醒行为/成本、endpoint origin、可信激活/driver、自动worker、真实POST→run/报告/unknown对账仍待完成；首次初始化恢复、备份fence、模板隔离、远端key撤销、有限重试/critical reserve及安全退役单独验证。 | `blocked`：[T43](T43-native-webhook-contract.md)/[T45](T45-template-webhook-delivery.md)/[T46](T46-template-ops-pairing.md)/[T54](T54-ops-targets-and-routing.md)剩余原生链与独立review未闭合；本次review503，当前原生源已换版，旧资格不直接沿用。默认driver unavailable，不以prepared或持有key签接收者可用。沿[CONFIG-CUTOVER](#live-config-cutover)固定旧制品/配置退路成套采用，不因J1或Git合并自动部署。候选回执：`docs/reports/2026-09-18-private-target-pairing.md`。 |
| <a id="live-ops-receivers"></a>**LIVE-OPS-RECEIVERS**<br>custom 接收者与有限路由 | **未运行现役Webhook或变更模型。** 候选5bd99af提供固定disabled blueprint和同帧Host能力/自动任务选模预检；专项117 pass，当前固定源函数探针1 pass/72断言，CLI732 pass。最终packages前两组1156 pass/20 skip，第三组86文件被工具拦截，未签全仓完成。 | Server所有权/账户scope、真实Webhook回合的模型/供应商/工具/数据与费用、可信激活/driver、POST→run/报告仍未证；preview不创建会话、不授权canary/发送。高级分流/备用/交接后置，unknown不广播。 | `blocked`：[T55](T55-custom-receiver-delivery.md)/[T43](T43-native-webhook-contract.md)/[T45](T45-template-webhook-delivery.md)/[T46](T46-template-ops-pairing.md)原生链仍有前置；独立审查本轮超时无报告，末组回归待执行。候选回执：`docs/reports/2026-09-18-receiver-model-preflight.md`。沿[CONFIG-CUTOVER](#live-config-cutover)固定旧制品/配置退路并成套采用，不因预检或J1自动部署。 |
