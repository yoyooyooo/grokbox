...
| 模型关系、隔离和实际effort | [选择](#live-model-selection)、[Bot隔离](#live-agent-isolation)、[SOL high](#live-model-sol-high)、[SOL xhigh](#live-model-sol-xhigh)、[Grok high](#live-model-grok-high)、[Grok xhigh](#live-model-grok-xhigh)、[DeepSeek high](#live-model-deepseek-high)、[DeepSeek xhigh](#live-model-deepseek-xhigh)、[Provider档位](#live-reasoning-provider)、[Host/App档位](#live-reasoning-host-app) | 保留E1指定的三模型两档全部六格和同一长会话；通道/模型实际资格先核对，不用一格替另一格，reported未知不伪报 |
...
| <a id="live-tool-contract"></a>**LIVE-TOOL-CONTRACT**<br>G1 | [ ] `partial`；`not-run` | ①每个 toolChoice 格和最终 name/schema/choice wire 对账；②captured-in-flight、terminal、permission denial、duplicate declaration/call 各留独立证据；③backend/wire/Host 层不跨 attempt 拼接；④Memory/episode 空结果、confirmed-overflow/compact 与下一输入 recovery 不伪造成工具成功；⑤真实 native schema、审批、执行、结果接受和送达仍逐项读回 | 来源与实施差额：[FIX-tool-contract-evidence](FIX-tool-contract-evidence.md) · [Host工具](../maintainers/host-inbound-agent-loop.md) · [上下文连续性](../maintainers/managed-context-continuity.md) |
...
| <a id="live-files-jobs"></a>**LIVE-FILES-JOBS**<br>G1 | [ ] `partial`；`not-run` | ①授权named root内stat/list/read/write/upload/download/mkdir/remove与hash读回；②越界路径/symlink拒绝；③job policy/start→list/get/wait/logs/cancel→原请求回执；④CLI退出、取消、管理Server重启后Job与输出限制符合合同，unknown不重放 | 来源与实施差额：[文件合同](../product-contract.md#8-云电脑文件命令) · [Jobs合同](../product-contract.md#9-云电脑执行与-jobs) |
...
| 命令（精确leaf） | 主场景 | 补充范围 |
|---|---|---|
| `init`, `profile list`, `profile show`, `profile use`, `profile add`, `profile update`, `profile remove`, `profile capabilities`, `doctor`, `system identity get`, `connection list`, `connection get`, `connection check`, `connection set`, `connection delete`, `connection operation get` | [LIVE-CLI-CONNECTIONS](#live-cli-connections) | local/daemon/有资格remote分别取证；本次网络边界变化另见[NET](#live-network-boundary) |
| `config get`, `config set`, `config unset`, `config apply`, `config validate`, `config schema`, `config path`, `config export`, `config preset`, `config aliases`, `config recover`, `system config get`, `system config export`, `system config apply`, `system config reset`, `system config operation get` | [LIVE-CONFIG-EDIT](#live-config-edit) | 写确认/revision及纯读取拒副作用 |
...
| `model list`, `model get`, `model apply`, `model delete`, `bot model get`, `bot model set`, `bot model reset`, `model default get`, `model default set`, `model default reset`, `models check`, `models persist-key`, `model check`, `model credential get`, `model credential import`, `model credential operation get`, `model probe`, `model probe-operation get` | [LIVE-MODEL-SELECTION](#live-model-selection) | 选择与维护已接 Server；probe 单次有界且可能计费，unknown 查原回执；旧本机维护入口的完整收束归 AH-144 |
...
| `quota`, `box status`, `box wake`, `box keepalive run`, `box keepalive status`, `system desktop get`, `system desktop prune`, `system desktop keep set` | [LIVE-OPTIONAL-CAPABILITIES](#live-optional-capabilities) | 桌面已迁入管理Server；读取、策略、回收及原操作权限分开，desktop 配置由既有策略 owner 发布；实际helper/平台资格另验 |
| `system access list`, `system access get`, `system access grant`, `system access revoke`, `system access operation get` | [LIVE-NETWORK-BOUNDARY](#live-network-boundary) | installation owner 管理受限委托 grant；读取不含 verifier，最终发布复核主体/权限，不改变网络部署 |
| `recover` | [LIVE-NETWORK-BOUNDARY](#live-network-boundary) | 默认应用恢复与显式旧映射恢复选项分开，网络故障不自动wake |
| `system host get`, `system integration get`, `system integration operation get`, `system integration reconcile`, `system host health`, `runtime profile analyze`, `runtime profile observe`, `runtime profile propose`, `runtime profile prune`, `runtime profile replay`, `runtime profile status`, `runtime profile watch`, `runtime profile write`, `runtime operation-recovery`, `runtime watchdog run`, `runtime contracts` | [LIVE-HOST-CAPABILITY-RECOVERY](#live-host-capability-recovery) | Host健康读面已接Server/Rust/原OBS，显示静态范围与未证运行见证；完整能力/实际加载仍待资格，watcher和recovery非另一自动部署器 |
...
| `runtime start`, `runtime status`, `runtime modeld replace`, `runtime modeld stop`, `runtime modeld status`, `runtime modeld run` | [LIVE-MODELD-RESTART](#live-modeld-restart) | owned与borrowed不同 |
