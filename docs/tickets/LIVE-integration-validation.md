...
| <a id="live-config-consumers"></a>**LIVE-CONFIG-CONSUMERS**<br>消费者、重启与模型路径 | **既往所选消费者已证，新storage采用未证。** W17的desktop/daemon及所选模型凭据保持原范围。2026-09-18只读modeld为wire8可达、采样activeSteps=0，Host协议匹配仍not_observed，不构成schema4或持续idle保证。 | schema4下monitor/process捕获revision、GC与通知关闭独立、旧执行/模型选择保持；journal配置及全安装storage applied尚有源码前置，不能用局部启动回执签全域。未选凭据不扩大探测。 | `blocked`：先完成[T51](T51-ops-capability-presets.md)/[OBS-04](OBS-04-bounded-observation-storage.md)对应owner与review，再与[CUTOVER](#live-config-cutover)同窗验收。配置committed/effective-intent不当作applied。[T58](T58-config-command-single-writer.md) · [T60](T60-config-ops-integration-proof.md) · [W17 §4](../reports/2026-09-17-live-integration-window.md#4-迁移与消费者) |
