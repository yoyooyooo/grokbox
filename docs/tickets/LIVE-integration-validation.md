...
- **消化的源码基线：`feat/box-runtime-v2 @ 7deeeac`，整合时吸收至`aad29d5`的官方duplicate增量。** config 4 / models 2 / wire 8；Routine、配对、显式发送、持续提醒授权和 daemon sender 已集成。collector 服务内装配已在后续观测切片接回；OS开机安装、完整跨 owner 配额、长期安全退役等仍有源码缺口，不能写成“只剩 live”。真正开窗时重新冻结当时 v2 的精确 SHA、制品、原生版本、配置 revision 和能力清单。
...
| <a id="live-obs-safe-retirement"></a>**LIVE-OBS-SAFE-RETIREMENT**<br>G2/G3 | `integrated/partial`；`blocked` CODE；通知恢复守卫和payload/unknown guard解耦已合入，完整退役未齐 | ①被引用恢复闭包/最后可靠点不被诊断GC删除；②旧STEP/commit_unknown/notify unknown/provision guard重启后仍拒重复副作用；③细节回收与安全标记退役分开；④容量满保护业务语义，备份恢复fence实际有效 | CODE：执行/context/provision的最终退役与全域恢复仍未齐，不删除账本腾位置。见[过期材料边界](../reports/2026-09-19-diagnostic-admission.md#过期材料不永久绑定未知副作用)；[OBS-05](OBS-05-safe-state-retirement.md) · [CONT存储](CONT-02-continuity-snapshots.md) |
