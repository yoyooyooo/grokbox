...
| <a id="live-host-capability-recovery"></a>**LIVE-HOST-CAPABILITY-RECOVERY**<br>必需桥能力、局部升级与中断恢复 | **未运行本轮现场验收。** 历史local-only读回/现役ready不是本轮能力声明、恢复和升级闭环证明。 | 实际旧/新wrapper与reader组合的doctor指引；官方局部profile升级及加载读回；adopt中断、双锁恢复和attestation重新提交；原App细因及一条新STEP的Provider请求/结果。 | `blocked`：[HCR-01–04](README.md#host-必需能力与中断恢复--hcr-0104)实施、隔离测试及review；固定合入v2制品后另获维护/测试Bot/费用窗口授权。不得重放旧消息、用full List替代local-only或由ready推定成功。[Spec](../roadmap/host-seam-ops-recognition.md#capability-recovery)。 |
