...
|---|---|---|---|
| <a id="live-ops-routines"></a>**LIVE-OPS-ROUTINES**<br>原生 Routine / Payload / 模板隔离 | **本轮未写现役Routine或发Webhook。** 候选已具disabled provision与精确对账，本片补default目标、原SQLite额度/attempt预留、固定revision/body、二次配对核对与只读通知状态。专项87 pass；全仓2464 pass/19 skip/0 fail，含真实子进程强杀与packed Node读取。配对/发送使用owned adapter，不是实际原生交付；J1合同保留。 | 原生配对/凭据owner及HTTP认证、接收者模型资格、自动worker安装、真实POST→run/报告/unknown对账、备份恢复fence与双模板隔离仍待完成；明确失败重试/critical reserve、批量routines-from及安全退役另有源码前置。 | `blocked`：[T43](T43-native-webhook-contract.md)/[T45](T45-template-webhook-delivery.md)/[T46](T46-template-ops-pairing.md)/[T54](T54-ops-targets-and-routing.md)剩余原生链与独立review未闭合；本次review仍503无结论。默认driver unavailable，不以配置目标或native-accepted字段签Bot完成。沿[CONFIG-CUTOVER](#live-config-cutover)固定旧制品/配置退路成套采用，不因J1或Git合并自动部署。候选回执：`docs/reports/2026-09-18-notification-outbox.md`。 |
