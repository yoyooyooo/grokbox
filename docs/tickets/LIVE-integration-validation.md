...
| <a id="live-obs-storage"></a>**LIVE-OBS-STORAGE**<br>长期容量与物理空间回收 | **本轮未运行现役压力/切换。** 8835fcf证据/SQLite局部容量/采集调度已线性合入v2并复验39项通过；modeld写入端有界日志与可丢弃Node正式replacement/fd检查另已离线证明，但日志增量仍在独立工作分支。旧raw文件未改，全安装预算未生效。[存储增量](../reports/2026-09-18-observation-storage-followup.md) · [日志回执](../reports/2026-09-18-modeld-process-log-rotation.md)。 | 实际journal/process fd轮转、SQLite含索引/辅助文件、manifest/租约/孤儿在长期高基数故障下稳态；ops通知关闭后GC继续；reader并发、崩溃、磁盘压力与gap；无用户数据误删。 | `blocked`：[OBS-04](OBS-04-bounded-observation-storage.md)/[OBS-06](OBS-06-integration-and-soak-proof.md)/[T50](T50-template-ops-release-proof.md)尚缺跨owner预算、结构化journal/其他producer轮转和常驻组合。8835fcf的类型/构建/专项门已通过；日志增量最新构建和56项测试通过，但最终类型检查被工具拦截，指定独立复核请求503均未算通过。先补相应门禁再固定v2制品验现役，不在生产填满盘或清历史。 |
