...
| <a id="live-obs-storage"></a>**LIVE-OBS-STORAGE**<br>长期容量与物理空间回收 | **本轮未运行现役压力/切换。** modeld日志98b62ea已完整复验并合入v2；新增结构化journal分段/游标/缺口和固定证据存续已通过离线及打包Node证明，全仓2317 pass/15 skip/0 fail。全安装预算仍未生效。[日志回执](../reports/2026-09-18-modeld-process-log-rotation.md) · [分段回执](../reports/2026-09-18-structured-journal-rotation.md)。 | 实际Host采用新日志leaf、journal/process轮转与真实文件系统长期稳态；SQLite辅助文件及全安装预算、通知关闭后GC继续、硬崩锁/撕裂索引恢复、reader/磁盘压力与无用户数据误删。 | `blocked`：[OBS-04](OBS-04-bounded-observation-storage.md)/[OBS-06](OBS-06-integration-and-soak-proof.md)/[T50](T50-template-ops-release-proof.md)仍缺跨owner预算、其他producer/安全退役和常驻组合。类型/构建不再受阻；指定独立复核本次被工具拦截，未取得报告。先完成相关源码/原生资格与review，再固定v2制品逐项采用，不在生产填满盘或清历史。 |
