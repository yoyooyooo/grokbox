...
| <a id="live-continuity-material"></a>**LIVE-CONTINUITY-MATERIAL**<br>分档保全与重建 | **无live采集。** [真实CONT存储组合](../reports/2026-09-18-continuity-recovery-store.md)及[原生checkpoint格式/全图读回](../reports/2026-09-18-continuity-native-checkpoint.md)已有限定证明；后者用实际Host/worker的schema/AgentStore和owned文件，不是实际Bot快照。 | 原生worker预算读取、checkpoint/GC共同边界的正式capture binding；完整Memory/展示历史/附件、四档config与自动触发、Temporal增量；实际源删除独立性和全安装存储预算。 | `blocked`：[CONT-02](CONT-02-continuity-snapshots.md)/CONT-00/07补真实读取边界及独立review，固定v2组合再验。原生格式与摘要归档已取证，不再以源码路径不可读阻断；仅matching root或普通export不代表完整恢复。 |
...
| <a id="live-current-context"></a>**LIVE-CURRENT-CONTEXT**<br>唯一当前状态控制 | **未安装官方binding。** [105项协调程序证明](../reports/2026-09-18-continuity-current-state.md)之外，新增[独立原生格式/writer/readback资格](../reports/2026-09-18-continuity-native-checkpoint.md)，含新Node读取owned目标；不是整Host重启或真实Agent loop。 | 实际目标准备屏障、原生数据库/worker写入与持久application marker、跨身份绑定；首轮真实请求/后续轮、reset防旧历史复活、self-request/迟到输入、Memory/历史导入。 | `blocked`：[CONT-07](CONT-07-current-context-control.md)/CONT-00继续完成正式acquire/initialize/marker接口，再独立review/固定v2窗口。原生reset可静默回空且metadata失败可发生在blob写入后，故不能以不报错或指针相同判成功；对账不等于可激活。 |
