...
`/host-health`已分开显示磁盘候选、运行编译、同代原引用注册和有界实际边界，读取不采用profile或触发模型。当前角色/配对未资格仍保持unknown/unsupported，不因61片apply通过显示整体健康。固定窗口与原生/合成范围见[来源适配报告](../reports/2026-09-20-host-idle-layout-adaptation.md)。
