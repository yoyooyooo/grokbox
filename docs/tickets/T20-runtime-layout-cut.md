...
- `packages/box-runtime/package.json`、`src/runtime.ts`、`src/preload.ts`、规格 S2 的 host/io/process/roots 落点；按 S7 搬保留机制，移除旧推理执行链/根 index barrel。
...
1. `bun scripts/verify-runtime-rebuild.mjs layout` → 实际结构检查 + `bun test packages/box-runtime/test/architecture.test.ts`。checker 解析 import/re-export/require/dynamic import/exports；故意的跨层 edge、旧 alias、第二入口在 virtual fixture 中必须非零。
...
A3fu 修复、provider enable、完整 inference/controller、WebUI、live adopt、改现役配置/fd、root `src/` 清理。新单轨未接通期间禁止部署。
