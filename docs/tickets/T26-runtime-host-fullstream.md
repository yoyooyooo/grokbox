...
Preserve the mixed-history Bot as a counterexample and keep clean-Bot production acceptance separate. No store merge, profile rewrite, Bot deletion or Host re-adopt is part of this observation change. Host-only: there is no App routing patch. Remaining work is V16 observation on the real desktop (pro): App-originated nonce must correlate to Host journal on a box-classified send; cached restore / late server roster / legitimate temporal are negative controls. If App still bypasses Host after Server/local agree, T39 records that App version incompatible. CLI `--expect-harness` does not certify desktop replica.
