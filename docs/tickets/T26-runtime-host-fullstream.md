...
Preserve the mixed-history Bot as a counterexample and keep clean-Bot production acceptance separate. No store merge, profile rewrite, Bot deletion or Host re-adopt is part of this observation change. Host-only: there is no App routing patch. 2026-09-13 pro V16：App→Host 已接通（nonce `45edf5b1` / hook_enter / expected_result_observed / roster box）。失败面是详情：replicas 未写入该 TURN，发送队列停在 `accepted-awaiting-echo`，列表能看到最新一句。CLI `--expect-harness` 仍不认证 desktop replica。缓存 restore / 重连 / 晚到 roster / 合法 temporal 负例未做。若详情 echo 不能在不改 App 的情况下闭合，T39 记该 App 版本显示路径缺口，不是发送没进 Host。Cmd-Q 后同 nonce 的 send-journal 出现 failedAtMs，详情改成「发送失败」；Host 记录仍在。禁止把重新发送当修复。
