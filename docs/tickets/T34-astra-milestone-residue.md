...
- Live test2 canary-on disconnected Host CF (eventCount 0 / attempt 0 / no resume). Canary env left unset. Compact-slot bind landed offline; live re-adopt this pass is `commit-failed`, not overflow surface.
...
- Disposition: receipt closures are acknowledged on their named cases. New tip evidence narrowly reopens P1-02/P1-04 below. **P2-01 closed later on `d5ec95d`** (offline source removal / fail-closed stubs; not a live admission claim). P1-01 is closed on the held inspect cases; P1-03/P1-05 remain closed. Owner end-of-rebuild triage only: no grok/inv-pi handoff, main-chain pause, automatic owner review, or further review loop.
...
- **P2-01 — closed** on `d5ec95d` (offline source removal / fail-closed stubs; not a live admission bypass). `controller.runtime.ts` no longer has LegacyWitness or a watchdog/manual tick body; `runWatchdogTick` / `runWatchdogCutover` / `runManualReadopt` throw `invalid_usage`. `live-inject.ts` no longer imports h3-live or `signalIfMatch`; inject/deactivate stay blocked with zero fake-tree signals. Control gate `legacy-executor-removed.test.ts` proves CLI/control composition do not mention the retired symbols. P1-02 lock ownership and P1-04 checkpoint-then-interrupt remain held below.
  - Evidence: `packages/box-runtime/test/legacy-executor-removed.test.ts`; receipt `PRIVATE_EVIDENCE`. Original SHA-pinned finding: first T28 review P2-01 and `PRIVATE_EVIDENCE`.
