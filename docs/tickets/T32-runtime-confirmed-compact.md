...
## Live residual
Near-`SNAPSHOT_JSON_MAX_BYTES` fills fail `stream_invalid` (`normalize/stream_shape`, eventCount 0) before CCS overflow codes. Fail site is modeld SDK `fullStream` unfinished (empty/skip-only), not Host reshape. test0 transcript polluted; further overflow canary is **test2** (`harness=box`). Filling to the 4 MiB wire cap is not an overflow recipe. `harness=temporal` skips the patched `createSession` and yields no managed STEP. Bounded CCS luna ping and GATE-off test2 pong still complete **ok** with no overflow codes; remaining live proof is owner policy on large-context spend below the snapshot cap, not a tiny-W invention.
