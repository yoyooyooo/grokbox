...
依赖[CTX-00](CTX-00-pi-compaction-reuse.md)的算法adapter/投影边界与[CTX-01](CTX-01-context-policy-and-meter.md)冻结budget/policy/identity，复用T35已有provider前slot、root guard、错误/outer-retry边界、S10 authority/Scope与F1/F2状态保真。T35未完成的pending/实际接受资格进入本票，不开第二施工路线。可先使用同合同无网络摘要替身验证owner；真实摘要与费用限制由CTX-03接入，不能用替身签整个能力完成。

## Module / change set

- 新 `runtime-kernel/internal/inference/context-maintenance.ts` 为唯一操作程序，消费CTX-00的 `ContextCompactionAlgorithm`，不再实现一份Pi算法；演进既有 `HostCompact` 为权威材料/合法分区/accept/readback能力，显式root版本/operation身份，无并行旧执行器。
...
4. CTX-R02的有界Pi只读视图/候选须逐项回映射Host sourceRef、合法工具组与控制metadata，不将retainedTail或Pi虚拟ID当作持久root。候选在活跃root修改前完成结构/目标验证；非法/空/仍超预算候选不覆盖旧root。prepare/summary/validate/accept前后取消、root版本改变、Host换代、正式归属撤销均有writer侧fence；持有旧mutator引用的迟到callback也无法写新root。
...
不从UI/store.db/整个archive重建第二prompt，不另写会话数据库或实例化Pi Session/AgentHarness，不让kernel或Pi算法直接修改Host文件，不复制原生私有代码到公共fixture。Pi算法暂时视图不是第二个历史writer；原生metadata保留与提交仍只归Host。不得把Promise.race取消当root回滚、Scope当事务、消息数减少当候选成功，或把现有抑制摘要的patch本身当维护能力。摘要模型实现、命令最终体验与live验收分别交CTX-03/04。
