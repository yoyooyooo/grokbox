...
最终代码候选 `b92b7d9198457dcf5720cb2dcafaa9828e7bda88`。独立审查原先指出不能依赖继承的PWD；修正为pwd -P并用尾部标记保留路径内换行，同一未参与实施的审查者复查明确Accepted，原finding关闭。新增目录末尾换行、错误PWD和相对输入反例保留。

在该固定候选上重新完成完整build、8文件124pass/0fail（包含shim8、内部Server36/product53/创建6，不重复累计）、根/Web类型、文档、包边界、发布扫描及clean检查。原state40与native-runtime四组在前一源码快照631fe837上完成并通过；此后只调整source-shim安装器和其测试，原运行时代码未变，不能冒充新的全量来源摘要上重跑。这两项均不签J2或真实Host/modeld采用。

源码shim的自动dotenv发现禁用是明确行为策略，导出环境变量与真实调用目录保留；未更新用户全局shim。Git和Linear实际合流回执拥有最终交付状态。
