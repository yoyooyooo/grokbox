...
A 的依赖动作仍需要原创建的可验证结果，或明确调整试验身份；不能靠改台账或另换请求 ID 把原未知操作重做。后续源码核对发现，现有 SQL 把一条未知创建扩大成同类创建全部禁止，这不是原生平台不能创建其他对象的证据。源码已收窄到原请求及相同规范化创建声明，同时保留已有对象的精确 targetId 围栏；独立不同声明可以推进。另补充有界的私有 HTTP 失败详情和可公开的阶段/状态摘要，诊断不结算 unknown。Node HTTP/SQLite 夹具证明原未知不变、相同声明不能换 UUID 重发、独立对象可创建，并覆盖已发生副作用后的 HTTP 错误与诊断提交不确定性。该源码修正尚未替换上述现场固定安装，不能反推旧 A 的失败原因或代签 E2E。私有来源、原生诊断、nonce 只读检查、全部失败与回执继续留在仓库外。下面保留历史演进与证据边界。
