...
Controller and identity writers share one current Linux advisory-gated operation lease. Recovery inspects without creating roots/gates; explicit confirm acquires controller then identity gates, checks the operation's own complete pid/start/uid/boot/nonce identity and unchanged protected file instance, and restores running metadata only to unknown. It does not emit signals, attest or replay the operation. PID reuse is judged against that captured identity, never PID alone. PID-only records and ownerless running rows remain untouched and blocked even when a stale lock exists; there is no legacy recovery or alternate lock writer.

`runtime operation-recovery --restore-operation <original-id> --confirm` separately records observed physical restoration of a failed unknown adoption under those same gates. It requires the original owner to be gone, the journal and marker to agree, the recorded temp supervisor and marker-bound Host to be gone, no surviving guardian, and a unique stable direct official chain with matching Gateway and no relevant preload. It requires complete owner-created temp/Host/guardian/holder PID/start provenance consistent with the original row, failure journal, compile receipt and preload digest; marker compilation alone is insufficient for legacy recovery. Exact recorded owners are checked independently of current argv/role. Stable readable discovery snapshots may exclude non-candidates without executable-link access, but ambiguous candidates and official preload reads remain strict. It freezes and rechecks the original ledger/journal/marker/old-attestation bytes immediately before publication. A durable prepared receipt remains incomplete until a separate completion link follows readback and final proof; inspection blocks incomplete evidence. The separate receipt preserves all original evidence and unknown rows; it proves neither adoption success nor permission to replay. Commit ambiguity, changed evidence, live or unreadable identities, and an existing receipt refuse publication. The same command without `--confirm` reads the historical receipt (`outcome: recorded`, `restorationHistorical: true`) without acquiring gates or revalidating current Host state. It does not consume the receipt as adoption admission; the original ID stays unknown. A distinct later operation cannot bypass an unresolved claim on the same adoption resources, including one preserved in a legacy pending journal. Completed original recovery can discharge that resource boundary; historical unknown rows alone are not a global prohibition. Operation journals and exact marker bytes are retained under the original operation independently of later singleton writes.

Adoption failure receipts retain the inner code, phase, actual signal/spawn facts, guardian disposition and bounded child exit/readiness evidence. Effect interruption aborts and joins the same adoption work and persists its actual result before releasing either lease. The guardian's existing deadline constrains subsequent waits and exact-owned-child cleanup. Attestation and journal publication use a synchronous syscall immediately between ownership checks; in-flight failure leaves the resource claim unresolved. If expiry prevents confirming child exit, the journal, controller row and external receipt record per-child cleanup as `unproven` with exact identity and observed signal/exit facts; a late exit cannot settle that unknown. Independent recovery still survives parent death. Raw child stdout/stderr, exception bodies, credentials and native source are excluded. Compilation and Gateway readiness remain separate observations.
