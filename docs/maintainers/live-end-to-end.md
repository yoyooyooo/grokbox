...
```markdown
### <LIVE-ID> / <windowId> / <model-cell或子oracle>
候选：source -> v2 -> tarball -> 实际loaded；配置/原生版本与时间范围。
输入：新nonce/operation/work引用，测试对象角色，实际依赖，预算。
动作：正式产品入口、必要参数类别（无secret）、安全注入范围。
观察：首个状态、后续状态、独立存储/工具/App读回。
结果：passed / failed / blocked / not-run / excluded；只描述实际已执行。
局限：Provider reported、App可见、未知副作用等不能从其他层推断。
恢复：已完成操作、未结记录、cleanup_required、保留证据与下一动作。
关联：来源修复票、被影响LIVE IDs、独立回归测试、旧失败不被覆盖。
```

维护者可以分多轮追加报告，但当前是否已通过只更新LIVE索引。新worktree使用LIVE里的追加模板，不新建自己的长期live-todo文件。
