...
| 首次限定真实闭环 | 固定候选上的配置/管理面、受控采用、专用双 Bot 原生 DM、工具唯一效果、真实 compact/follow-up 与清理回官方；使用 [T40 受控窗口](../tickets/T40-persistent-release-and-rollback.md#首次受控窗口) | 不代签完整模型矩阵、App 体验、重启自启或日用 |
...
- 及早确认目标 Box 的真实服务宿主。旧 systemd 适配和环境报告是来源，不证明当前环境；持久服务宿主不可用时，持久/重启场景保持阻断。首次限定闭环可使用 T40 的正式前台 Server、显式 modeld replace 与精确 stop 恢复合同；不能临时 nohup 或擅改官方 supervisor。
