...
Current compaction behavior is owned by [context maintenance](../runtime/context.md) and the actual Host/modeld source. Default-auto policy and qualified root/operation capabilities replace the old normal-function environment gate; they are implemented mechanisms, not proof that the loaded Host adopted them. Dedicated native summary, conversation-compaction and qualified memory/episode purposes retain distinct routing and identities. The original App must receive truthful maintenance activity and terminal states through Host events; no App patch, fake model text or perpetual Working. Fault injection remains separate and disabled by default. Historical canary permission is not standing rollout authority. Current field proof and gaps are maintained only in [LIVE-MODELD-APP](../tickets/LIVE-integration-validation.md#live-modeld-app) and [LIVE-CTX-ADOPTION](../tickets/LIVE-integration-validation.md#live-ctx-adoption); this page owns projection semantics, not live status.
