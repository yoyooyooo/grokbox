...
| Operator-managed networking | no product-specific adapter | Explicit endpoint and optional installed-service SSH recovery; retired peer/bootstrap/Serve routes and config are not accepted |
...
storage, configured endpoint behavior, or Grok Bot filesystem/process layouts change.
