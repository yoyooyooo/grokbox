...
| --- | --- | --- |
| **J0 共同基线** | Q；已提交管理成果与排程文档 | v2 包含原 `fc346bca` 和本排程提交、工作树干净，docs/实际命令映射按该树检查。其后新开发分支全部从当前 v2 切出。不等桌面未提交实现或 VOICE 规划 |
...
1. J0将已提交管理成果和本波文档线性合回v2；如v2仍为祖先则`--ff-only`，有新提交才先在施工分支rebase并复验，不制造无意义rebase。
...
- **桌面未提交工作**：仍由原施工worktree单一负责人保留/完成定向修复、旧消费者和浏览器复验，单独回流；不是其他路线J0的前置。F只接完成的出口，未交接前不得并行重写同一桌面文件。原Bot删除后的seat cleanup仍须由D/F明确归属。
- **VOICE规划**：原8个暂存文档独立保留，不混入本次拓扑提交，也不增加核心/完整产品当前门槛。
