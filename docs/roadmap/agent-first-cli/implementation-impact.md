...
状态：实施中，模型关系/回执、共享客户端与管理 Server 已接通首批 CLI 用例，当前差额见 [CLI-05](../../tickets/CLI-05-implementation-follow-through.md)。CLI 合同是前置，交付范围覆盖统一后台、已有必要能力重建、Web UI 与最终安装验收。本页固定责任、工作链路及完成条件；具体包名、schema、路由和组件由实施者决定。产品语义与数据兼容边界唯一见 [Spec](spec.md)。
...
| W3 能力与功能页面 | T41、CONT、模型、DATA-01 交付领域差额，WEB-03 同步接入朴素的真实页面；CLI-05 迁入消费者并退出旧 writer，不等待视觉定稿 |
| W4 完整功能候选与集中 E2E | 完成安装、服务生命周期、数据接入、适用整体检查和独立审查；固定实际制品后按 LIVE 验收真实 CLI/API、原生往返、功能浏览器、恢复与有界持续运行 |
| W5 用户验收与最终呈现 | 用户集中验收后才进入日常吃狗粮；WEB-01 定稿后补相应视觉/体验验收，实际命令文档与 Skill 同步；正式发布另行决定 |

这些是依赖关系，不是五次对用户发布。具备用例合同的独立工作可推进，不要求全部字段先一次性定完，也不要求每次中间提交都能供用户使用。及早接通真实用例用于验证责任边界；不得以长期 mock 页面、仅改 CLI 名字或保留重复业务 owner 代替完成。

开发期针对变化验证，不反复做旧版本采用/回退和完整模型矩阵。冻结候选时再收口适用整体检查；高风险原生能力、来源写入和进程宿主须提前小范围验证，不能推迟到最终 E2E 才发现根本不可行。局部报告不签整个系统可用。CLI-05 的最终收束与 [LIVE 的 E0–E6](../../tickets/LIVE-integration-validation.md#window-order)分别是施工责任和验收路线，不相互替代。
