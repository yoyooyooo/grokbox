...
- Deliver the rebuilt product with one current implementation and contract per capability. Migrate required behavior, then retire superseded entry points, writers, compatibility branches and refusal-only stubs with their consumers. Do not preserve an old execution path to make historical tests pass. Preserve user data and unknown-operation evidence; preservation is not permission to reinterpret them as current success or to keep live compatibility.
