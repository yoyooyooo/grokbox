...
The receipt parser is in `scripts/live-validation.mjs`. Its schema acceptance does not establish the scenario's oracle. A receipt is `eligible` only when it binds fixed candidate and actual loaded identities, records authorization/cost/time bounds, names native or native-isolated evidence, and carries an independent review from someone who did not implement the work package. Fixture/fake evidence, synthetic or unknown loaded identities, self-review, missing loaded identity, unknown results, or remaining `notProven` items remain `structural-only`; they must never be reported as live pass. Record bounded observations and what remains unverified, without credentials, raw transcripts or provider bodies.
