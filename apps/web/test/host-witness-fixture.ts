...
export async function hostWitnessFixture(origin: string, ports: HostHealthTestPorts = {}) {
  ...
  try {
    ...
    process = await launchPublicHost(f);
    ...
    const discoveryPath = join(f.root, "witness-native.json");
    ...
    const gateway = createManagementGateway({ discoveryPath, configurationRoot: f.root }); reader = gateway.readHostWitness;
    const baseUrl = `http://127.0.0.1:${port}`;
    return { ...f, get server() { return f.server; }, probeState: state, process, baseUrl, discoveryPath,
