...
function Overview() {
  ...
  return <><Heading eyebrow="WORKSPACE OVERVIEW" title="Box 概览">先看来源与状态，再进入具体对象。此页不是一个替代所有资格检查的绿灯。</Heading>
    ...
    <Card title="能力与证据边界"><div className="capability-grid"><div><h3>模型与操作</h3><p>查询、配置、默认关系和逐 Bot 选择进入共享领域用例。保存后可通过原 request-id 查询回执。</p><Link to="/models" search={{}}>管理模型 →</Link></div><div><h3>观察与资料</h3><p>此切片展示当前原生 Bot 快照。持续事件、全局材料、运行日志与保护交接尚未在此页面接入，不显示虚构数据。</p><Badge tone="warn">仍有领域接入差额</Badge></div><div><h3>执行与寿命</h3><p>此页面未观察当前 TURN 捕获、modeld 进程或 Host 加载资格。管理 API 可读不能替代它们的现场证明。</p><Link to="/operations" search={{}}>查询操作回执 →</Link></div></div><ErrorNotice error={viewError(models)}/></Card></>;
}
