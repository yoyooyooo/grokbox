...
type Receipt = import("@grokbox/client").DesktopOperation | import("@grokbox/client").DesktopPolicyReceipt | FileOperation | JobView | JobCancelReceipt | HandoverOperation | CompactionOperation | ModelOperation | IncidentOperation | ReceiverOperation | NotificationSendOperation | NotificationTestOperation | SetupOperation | MaterialOperation | ProtectionOperation | ContextOperation;
...
export const Route = createFileRoute("/_console/operations")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    ...
    domain: ["desktop", "desktop-policy", "file", "job", "job-cancel", "handover", "compaction", "context", "protection", "material", "incident", "receiver", "notification", "notification-test", "notification-settings", "routine", "pairing"].includes(String(search.domain)) ? search.domain as Search["domain"] : undefined,
...
function receiptDomain(data: Receipt): OperationDomain {
  if ("botDeleted" in data) return "desktop";
  if ("changedPaths" in data) return "desktop-policy";
...
function receiptTarget(data: Receipt, installation: string): string {
  if ("botDeleted" in data || "changedPaths" in data) return `desktop:${installation}`;
...
function ReceiptFacts({ data }: { data: Receipt }) {
  if ("botDeleted" in data) return <><dt>Desktop batch</dt><dd>{data.origin} · {data.rows.length} display instances</dd><dt>Observations</dt><dd>{data.rows.map(r => `Display ${r.display}: ${r.state}`).join("; ") || "No eligible candidates"}</dd><dt>Meaning</dt><dd>No Bot is deleted; display observations are not an atomic seat lease. Unknown helper effects are never replayed by this read.</dd></>;
  if ("changedPaths" in data) return <><dt>Desktop policy publication</dt><dd>{data.state}</dd><dt>Original revisions</dt><dd><code>{data.beforeRevision} → {data.revision}</code></dd><dt>Application</dt><dd>Consumer adoption is not observed by this historical configuration receipt.</dd></>;
...
function Operations() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const row = localOperations(localStorage, scope).find(item => data && item.requestId === data.requestId
        && operationDomain(item) === receiptDomain(data) && item.target === receiptTarget(data, scope.installationId)
  ...
  const data = receipt?.data;
  const title = data ? "botDeleted" in data ? "Original desktop reclaim" : "changedPaths" in data ? "Original desktop policy" : "serviceGeneration" in data ? "Original file operation" : "jobRef" in data ? "job" in data ? "Original cancellation receipt" : "Original Job receipt" : "notificationRef" in data ? "Original incident delivery receipt" : "handoverRef" in data ? "Original handover operation" : "nativeSettlement" in data ? "Compaction operation receipt" : "sourceBodyIncluded" in data ? "Current context operation" : "nativeEffectsPerformed" in data ? "Protection policy receipt" : "sourceWrite" in data ? "Source operation receipt" : "kind" in data ? "Setup operation receipt" : "receiverRef" in data ? data.action === "test" ? "Independent test receipt" : "Receiver consent recorded"
  ...
  return <><Heading eyebrow="COMMIT ≠ EXECUTION" title="操作回执">使用原 request-id 与领域定位查询。异常、接收者及测试回执还需原数据库身份；未知结果只查证，不自动重放。</Heading>
    <Card title="查找原操作"><form onSubmit={lookup}><div className="toolbar"><label htmlFor="operation-domain">领域</label><select id="operation-domain" value={domain} onChange={event => setDomain(event.target.value as OperationDomain)}><option value="model">模型配置</option><option value="desktop">Desktop reclaim</option><option value="desktop-policy">Desktop policy</option><option value="file">Source file publication</option><option value="job">Job execution</option><option value="job-cancel">Job cancellation</option><option value="context">Current context</option><option value="compaction">Compaction</option><option value="handover">Handover</option><option value="protection">Protection policy</option><option value="material">Source document</option><option value="incident">异常管理</option><option value="receiver">Receiver consent</option><option value="notification">Incident notification delivery</option><option value="notification-test">Independent notification test</option><option value="notification-settings">Notification settings</option><option value="routine">Routine setup</option><option value="pairing">Private pairing</option></select></div>
      ...
      {data && <div className="receipt" data-testid="operation-receipt"><Badge tone={data.state === "unknown" ? "warn" : "info"}>{data.state}</Badge><h3>{title}</h3><dl>
        <dt>操作引用</dt><dd><code>{"operationRef" in data ? data.operationRef : "jobRef" in data ? data.jobRef : data.requestId}</code></dd><dt>请求标识</dt><dd><code>{data.requestId}</code></dd><ReceiptFacts data={data}/></dl></div>}
