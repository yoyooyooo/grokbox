...
import { contextOperationRef, type ContextOperation, UUID, type ModelOperation, type IncidentOperation, type ReceiverOperation, type NotificationTestOperation, type SetupOperation, type MaterialOperation, type ProtectionOperation } from "@grokbox/client";
import { Badge, Card, Empty, ErrorNotice, Heading, SourceTime, useConsole } from "../components/ui.tsx";
import { contextLocalState, forgetSettledOperation, localOperations, markOperation, operationSearch, operationDomain, type LocalOperation, type OperationDomain } from "../lib/operations.ts";
import { readView, viewError } from "../lib/views.ts";

type Receipt = ModelOperation | IncidentOperation | ReceiverOperation | NotificationTestOperation | SetupOperation | MaterialOperation | ProtectionOperation | ContextOperation;
...
export const Route = createFileRoute("/_console/operations")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    ...
    domain: ["context", "protection", "material", "incident", "receiver", "notification-test", "notification-settings", "routine", "pairing"].includes(String(search.domain)) ? search.domain as Search["domain"] : undefined,
  ...
  loader: ({ context, deps }) => {
    ...
    return readView<Receipt>(deps.domain === "context" ? Promise.resolve().then(() => client.contextOperation(contextOperationRef(context.bootstrap.binding.installationId, deps.scopeId ?? "", deps.requestId!))) : deps.domain === "protection" ? client.protectionOperation(deps.target??"",deps.requestId) : deps.domain === "material" ? client.materialOperation(deps.requestId) : deps.domain === "routine" ? client.setupOperation("routine",deps.bot ?? "",deps.requestId)
...
function Operations() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const row = localOperations(localStorage, scope).find(item => data && item.requestId === data.requestId
        ...
        && (item.command === "context-control" ? item.contextScope === search.scopeId : item.setupKind ? item.setupScope === (item.setupKind === "routine" ? search.bot : "installation")
          : receiptDomain(data) === "model" ? !item.databaseId : item.databaseId === search.databaseId));
      if (row && data) markOperation(localStorage, row, "sourceBodyIncluded" in data ? contextLocalState(data.state) : data.state);
  ...
  function lookup(event: FormEvent) {
    event.preventDefault(); if (!UUID.test(id.trim()) || ["incident","receiver","notification-test"].includes(domain) && !UUID.test(database.trim()) || domain === "routine" && !UUID.test(bot.trim()) || domain === "context" && !/^[a-f0-9]{64}$/.test(accountScope.trim())) return;
    void navigate({ search: { requestId: id.trim().toLowerCase(), ...(domain === "context" ? { scopeId: accountScope.trim() } : {}), ...(domain !== "model" ? { domain } : {}),
  ...
  const data = receipt?.data;
  const title = data ? "sourceBodyIncluded" in data ? "Current context operation" : "nativeEffectsPerformed" in data ? "Protection policy receipt" : "sourceWrite" in data ? "Source operation receipt" : "kind" in data ? "Setup operation receipt" : "receiverRef" in data ? data.action === "test" ? "Independent test receipt" : "Receiver consent recorded"
  ...
  return <><Heading eyebrow="COMMIT ≠ EXECUTION" title="操作回执">使用原 request-id 与领域定位查询。异常、接收者及测试回执还需原数据库身份；未知结果只查证，不自动重放。</Heading>
    <Card title="查找原操作"><form onSubmit={lookup}><div className="toolbar"><label htmlFor="operation-domain">领域</label><select id="operation-domain" value={domain} onChange={event => setDomain(event.target.value as OperationDomain)}><option value="model">模型配置</option><option value="context">Current context</option><option value="protection">Protection policy</option><option value="material">Source document</option><option value="incident">异常管理</option><option value="receiver">Receiver consent</option><option value="notification-test">Independent notification test</option><option value="notification-settings">Notification settings</option><option value="routine">Routine setup</option><option value="pairing">Private pairing</option></select></div>
      ...
      {data && <div className="receipt" data-testid="operation-receipt"><Badge tone={data.state === "unknown" ? "warn" : "info"}>{data.state}</Badge><h3>{title}</h3><dl>
        <dt>操作引用</dt><dd><code>{data.operationRef}</code></dd><dt>请求标识</dt><dd><code>{data.requestId}</code></dd><ReceiptFacts data={data}/></dl></div>}
