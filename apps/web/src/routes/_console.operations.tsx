...
import type { JobView, JobCancelReceipt, HandoverOperation } from "@grokbox/client";
...
import { jobLocalState, contextLocalState, compactionLocalState, handoverLocalState, forgetSettledOperation, localOperations, markOperation, operationSearch, operationDomain, type LocalOperation, type OperationDomain } from "../lib/operations.ts";
import { readView, viewError } from "../lib/views.ts";

type Receipt = JobView | JobCancelReceipt | HandoverOperation | CompactionOperation | ModelOperation | IncidentOperation | ReceiverOperation | NotificationSendOperation | NotificationTestOperation | SetupOperation | MaterialOperation | ProtectionOperation | ContextOperation;
...
export const Route = createFileRoute("/_console/operations")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    ...
    domain: ["job", "job-cancel", "handover", "compaction", "context", "protection", "material", "incident", "receiver", "notification", "notification-test", "notification-settings", "routine", "pairing"].includes(String(search.domain)) ? search.domain as Search["domain"] : undefined,
...
function Operations() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (row && data) markOperation(localStorage, row, "jobRef" in data ? "job" in data ? data.state === "unknown" ? "unknown" : "recorded" : jobLocalState(data.state) : "handoverRef" in data ? handoverLocalState(data.state) : "nativeSettlement" in data ? compactionLocalState(data.state) : "sourceBodyIncluded" in data ? contextLocalState(data.state) : data.state);
  ...
  const data = receipt?.data;
  const title = data ? "jobRef" in data ? "job" in data ? "Original cancellation receipt" : "Original Job receipt" : "notificationRef" in data ? "Original incident delivery receipt" : "handoverRef" in data ? "Original handover operation" : "nativeSettlement" in data ? "Compaction operation receipt" : "sourceBodyIncluded" in data ? "Current context operation" : "nativeEffectsPerformed" in data ? "Protection policy receipt" : "sourceWrite" in data ? "Source operation receipt" : "kind" in data ? "Setup operation receipt" : "receiverRef" in data ? data.action === "test" ? "Independent test receipt" : "Receiver consent recorded"
