...
function ConsoleLayout() {
  ...
  return <div className="app-shell"><aside className="sidebar" aria-label="主导航"><Link to="/" className="brand"><span className="brand-mark"><Boxes size={23}/></span><span>grokbox<small>CONTROL ROOM</small></span></Link>
    <p className="nav-label">WORKSPACE</p><nav><Link to="/" activeOptions={{ exact: true }}><Activity size={18}/>概览</Link><Link to="/bots" search={{}}><Bot size={18}/>Bots</Link><Link to="/models" search={{}}><Settings2 size={18}/>模型配置</Link><Link to="/operations" search={{}}><ShieldCheck size={18}/>操作回执</Link><Link to="/notifications"><Activity size={18}/>通知后台</Link><Link to="/materials" search={{kind:"memory"}}><Boxes size={18}/>Materials</Link><Link to="/protection" search={{}}><ShieldCheck size={18}/>Protection</Link><Link to="/host-health"><ShieldCheck size={18}/>Host health</Link><Link to="/contexts" search={{}}><ShieldCheck size={18}/>Context</Link><Link to="/lifecycles" search={{}}><Bot size={18}/>Lifecycles</Link><Link to="/jobs" search={{}}><Activity size={18}/>Jobs</Link><Link to="/observation"><Activity size={18}/>持续观察</Link><Link to="/incidents" search={{}}><ShieldCheck size={18}/>持久异常</Link><Link to="/events" search={{}}><Activity size={18}/>观察变化</Link></nav>
