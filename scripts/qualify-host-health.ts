...
for(let i=0;i<args.length;i+=2){const k=args[i],v=args[i+1];if(!k||!v||!['--source','--worker','--profile','--binary-directory'].includes(k)||values.has(k))throw Error('invalid_qualification_arguments');values.set(k,resolve(v));}
...
try {
 ...
 const {artifacts:a,analysis}=await inspectHostHealthSources({source:values.get('--source')!,worker:values.get('--worker')!,profile:values.get('--profile')??null},values.get('--binary-directory')!);
 const text=new TextDecoder('utf-8',{fatal:true}).decode(a.source);
 const groups=[['core',LIVE_SLICE_PATCHES],['checkpoint',NATIVE_CHECKPOINT_HOST_SLICES],['current-state',NATIVE_CURRENT_STATE_SLICES]] as const;
 ...
 console.log(JSON.stringify({scope:'disk-static-only',sourceSha:a.sourceSha,sourceBytes:a.source.length,workerSha:a.workerSha,workerBytes:a.worker.length,
  selectedProfile:a.profileDigest,selectedApplicability:a.applicability,completeCurrentRecipe:full.ok?{state:'applicable-not-reviewed'}:{state:'mismatch',code:full.code,sliceId:full.sliceId??null},diagnostic,
