...
export function assessIncident(rule: string, facts: readonly EvidenceFact[]): IncidentAssessment {
  if(rule==="host_patch_health")return {classifierVersion:INCIDENT_CLASSIFIER_VERSION,category:"host_compatibility",disposition:"notify",reason:"host_contract",basis:"derived",rootCause:"not_proven",basisRefs:facts.filter(f=>["host_patch_health","host_runtime_health","host_capability_health"].includes(String(own(f.value,"name")))).slice(0,16).map(f=>f.ref)};
