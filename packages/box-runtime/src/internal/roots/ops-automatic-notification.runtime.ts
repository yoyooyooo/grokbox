...
import { NOTICE_WORKER_POLICY, selectNotificationTarget, type NoticeAuthorization, type AutomaticNoticeCycle, type AutomaticNoticeWorkerStatus } from "@grokbox/runtime-kernel/observation";
...
type Input = { durableRoot: string; readNative: ExplicitReceiverReader; signal?: AbortSignal };
...
export async function automaticNoticeCycle(input: Input, ports: Ports = {}): Promise<AutomaticNoticeCycle> {
  try {
    ...
    const workId = await store.nextAutomaticNotification(auth.activatedAtMs, nowMs);
...
export function startOpsNotificationWorker(input: Omit<Input, "signal">, testPorts: Ports & {
  ...
  const program = Effect.scoped(Effect.gen(function* () {
    ...
    yield* Effect.forever(Effect.gen(function* () {
      ...
      const result = yield* Effect.uninterruptible(Effect.tryPromise({
        try: () => testPorts.cycle ? testPorts.cycle(controller.signal) : automaticNoticeCycle({ ...input, signal: controller.signal }, testPorts),
  ...
  return { status: (): AutomaticNoticeWorkerStatus => structuredClone(state), close: async () => { controller.abort(); await lifetime; } };
}
