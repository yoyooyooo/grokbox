...
import { canonicalOwnershipAgrees, type IdentityOpResult } from "../process/identity-op.ts";
...
import { countRoles, identitiesMatch, readOnlyProcessPort, singleOfficialChain, type ProcessIdentity, type ProcessPort } from "../process/process-port.ts";
import {
...
} from "../process/transient-adopt.ts";
...
async function resolveModeldReady(input: WatchdogTickInput, ephemeralRoot: string): Promise<boolean> {
  ...
  return await probeModeldHealth(ephemeralRoot);
}
