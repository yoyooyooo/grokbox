...
import { pickLaunchEnv } from "./launch.node.ts";
import { loadReviewedProfile, waitOfficialReplacement, type RoleClassifier } from "./official-chain.ts";
  ...
  return await runTransientAdoptDeactivate({
    ...
    waitReplacement: async (oldPid) =>
      await waitOfficialReplacement({
        ...
        processes: input.ports.processes,
        classify: input.ports.classify,
        hasGrokboxPreload: (host) => input.ports.hasGrokboxPreload(host),
        readGatewayPid: () => input.ports.readGatewayPid(),
        budgetMs: input.ports.waitBudgetMs ?? 8000,
      }),
    hasGrokboxPreload: input.ports.hasGrokboxPreload,
    readGatewayPid: input.ports.readGatewayPid,
    clearAttestation: async () => {
