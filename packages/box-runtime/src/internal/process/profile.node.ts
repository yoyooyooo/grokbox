...
function authoringSlices(value: unknown): SlicePatch[] {
  ...
  if (!Array.isArray(value) || value.length < 2 || value.length > 6) {
    invalid("Profile authoring requires two to six approved slices.");
  }
  ...
  const patches = value.map((slice: unknown) => {
    ...
    if (record.id !== "create-session" && record.id !== "agent-id" && record.id !== "compact-register" && record.id !== "activity-bridge" && record.id !== "memory-purpose" && record.id !== "episode-purpose") {
  ...
  if (!approvedSliceSet(patches)) invalid("Profile authoring requires two to six approved slices.");
  return patches;
}
