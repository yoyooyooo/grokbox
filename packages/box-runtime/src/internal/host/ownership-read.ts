...
import { OWNERSHIP_MAX_TARGETS, OWNERSHIP_WAIT_MS, OWNERSHIP_SERVER_CACHE_MS } from "@grokbox/runtime-kernel/contract";
...
export function bindHostOwnershipRead(options: { timeoutMs?: number; now?: () => number; cacheMs?: number } = {}) {
  ...
  return async (ports: ReadPorts) => {
    ...
    if (!Array.isArray(ids) || ids.length < 1 || ids.length > OWNERSHIP_READ_MAX_AGENTS
      ...
      return { schemaVersion: 1, observedAt, state: "invalid_request", agents: [] };
    }
    // Legacy unscoped readers remain diagnostic only; v2 execution refuses them.
    if (!ports.readScope && pending) return { schemaVersion: 1, observedAt, state: "busy", agents: [] };
    ...
    try {
      const work = async () => {
        ...
        if (scope && cache && cache.scope === scope && at >= cache.at && at - cache.at < cacheMs) {
          raw = cache.rows; serverAt = cache.at;
        } else {
          ...
          const entry = pending;
          try { raw = await entry.task; serverAt = entry.at; }
          finally { if (controller.signal.aborted) entry.expired = true; }
        }
        if (controller.signal.aborted) throw new Error("timeout");
    ...
    return {
      ...
      observedAt, completedAt: new Date(now()).toISOString(),
      ...
      source: "Host.official-client/ListGrokBotAgents", errorCode: failure,
