...
import { CurrentStateFailure, applicationObservation, continuityObject, continuityStorePolicy, currentStateRpcRequest,
  ...
  nativeCurrentHead, sameCurrentHead, isContinuityHash, type InitializationAttempt, type NativeCurrentHead } from "@grokbox/runtime-kernel/continuity";
...
export function createNativeCurrentStateRpc(owner: ReturnType<typeof createNativeCurrentStateOwner>, profileDigest: string) {
  ...
  return async (raw: unknown, readOwnership: () => Promise<unknown>) => {
    try {
      ...
      if (input.action === "preview") {
        ...
        return { ok: true, data: { inputDigest: attempt.inputDigest, candidateHash: sha256Text(canonicalJson(material.manifest.parts)), rootHash: root.hash } };
      }
