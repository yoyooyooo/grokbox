...
import { rootConfigLayout, readConfigFile, publishConfigFile } from "./config-layout.node.ts";
    ...
    saveModels: async (file, expectedRevision, beforePublish) => {
      ...
      try {
        ...
        if (expectedRevision !== undefined) {
          const current = persistModelsDocument(parseModelsFile(await readConfigFile(modelsPath(root), true)));
