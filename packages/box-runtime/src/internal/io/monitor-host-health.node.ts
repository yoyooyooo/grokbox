...
import {projectHostHealth,hostHealthConditions,projectHostRuntimeEvidence,hostRuntimeCondition} from "@grokbox/runtime-kernel/host-health";
...
export async function indexHostHealthCondition(db:MonitorSqlite,input:{rootId:string;value:unknown;ref:string;at:number;
  ...
  const disk=projectHostHealth(input.value), runtime=projectHostRuntimeEvidence(input.value), v=disk??runtime;if(!v)return;
  ...
  if(runtime?.observation.receipt){
    ...
    const r=runtime.observation.receipt;
    if(newest&&(String(newest.attempted_at)>r.at||String(newest.attempted_at)===r.at&&newest.attempt!==r.observationId))return;
  }
  const conditions=disk?hostHealthConditions(disk):[{cause:"compilation",result:hostRuntimeCondition(runtime!)}];
