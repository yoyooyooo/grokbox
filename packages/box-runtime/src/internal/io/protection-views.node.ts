...
import { continuityStorePolicy, isContinuityUuid, isContinuityHash, failContinuity, HANDOVER_KINDS, WORKFLOW_STEPS } from "@grokbox/runtime-kernel/continuity";
...
const phase = (v: unknown, values: readonly string[]): string => typeof v==="string" && values.includes(v)?v:failContinuity("integrity_failure");
...
export async function readProtectionHandover(root: string, scopeId: string, operationId: string) {
  ...
  return Effect.runPromise(owner.db.read(async db=>{
    const row=await db.first("SELECT kind,source_id,target_id,phase,created_at,updated_at FROM continuity_workflows WHERE operation_id=?",[operationId]);
    ...
    const duties=await db.all("SELECT item_id,kind,state,(result_json IS NOT NULL) AS has_evidence FROM continuity_handover_items WHERE operation_id=? ORDER BY item_id LIMIT 129",[operationId]);
    ...
    return {scopeId:owner.scopeId,operationId,sourceId:optionalId(row.source_id),targetId:optionalId(row.target_id),kind:phase(row.kind,["clone","replace","spawn"]),
