...
test("failed convergence reads persist a gap and a missing deletion fence can never delete the predecessor", async () => {
  ...
  try {
    ...
    let fail=false,deletes=0,now=Date.now()+120000;
    const control=openBotConvergence({durableRoot:root,scopeId,native:{authorize:async()=>true,
      observe:async()=>{if(fail)throw Error("unavailable");return {cursor:"c".repeat(64),observedAtMs:now,newMessages:0,contiguous:true,state:{lastEntryId:"last"},
        targetUsable:true,dependenciesVerified:true,resourcesIndependent:true,deletionFenceAvailable:false};},
