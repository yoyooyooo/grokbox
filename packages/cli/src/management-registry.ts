...
export const MANAGEMENT_COMMANDS: readonly LeafCommand[] = [
  ...
  command("operation get", "Read a domain receipt using the original request UUID and source locator.", undefined, [
    ...
    { flags: "--domain <domain>", description: "model (default), material, protection, lifecycle, context, incident, receiver, notification-test, notification-settings, routine, or pairing" },
    { flags: "--database-id <uuid>", description: "Required for incident/receiver/test receipts; use the original database identity" },
