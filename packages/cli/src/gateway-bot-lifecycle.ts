import { createCurrentStateClient, openContinuityCurrentState, openContinuityRecoveryStore, openRuntimeStore,
  changeRuntimeModel, openContinuityControls, type BotLifecyclePort } from "@grokbox/box-runtime/runtime";
...
import { nativeMaterialReader, botProfileRevision } from "./gateway-bot-material.ts";

const fail = (code: ConstructorParameters<typeof CurrentStateFailure>[0]): never => { throw new CurrentStateFailure(code); };
const generation = (d: Discovery) => canonicalJson([d.baseUrl, d.pid, d.startedAt]);
/** Formal adapters for the shared lifecycle. Raw material stays private; no
 * direct native file patch, harness setter or hidden user task is available. */
export function createGatewayBotLifecycle(deps: CliDeps, input: { timeoutMs: number;
  handover?: BotLifecyclePort["handover"];
  authorizePolicy?: (request: BotWorkflowRequest) => Promise<boolean> }) {
  const gateway = new GatewayClient({ ...deps, transport: "local" }), runtime = openRuntimeStore(deps.boxRuntimeRoot, deps.env);
  ...
  const native: BotLifecyclePort = {
    ...
    model: async (request, targetId) => {
      ...
      if(!desiredNow){
        ...
        await changeRuntimeModel({ store: runtime, forAgent: targetId,expectedSelectionHash:beforeHash,
          ...(request.modelRef === null ? {} : { modelId: request.modelRef, effort: request.effort }), ownershipRead: runtimeOwnershipReader(deps),
    ...
    compose: async (request, targetId, sourceSnapshotId, snapshotId) => {
      ...
      if (source?.manifest.quality === "native_checkpoint" && !source.manifest.gaps.includes("unknown_effects")
        && source.manifest.source.nativeSchema === target.head.nativeSchema) {
    ...
    initialize: async (request, targetId, snapshotId, operationId) => {
      ...
      let saved = await store.initializationRequest(operationId).catch(error => {
  ...
  const captureMemory = async (request: BotWorkflowRequest, snapshotId: string) => {
    if (!request.sourceId || !qualification) return fail("invalid_request");
    const prior = await published(request, snapshotId);
    if (prior) return describe(snapshotId, prior, prior.reference.revision);
    await ownerProof(request.sourceId, request.scopeId);
    const material = await reader.fallback(request, qualification);
    await ownerProof(request.sourceId, request.scopeId);
