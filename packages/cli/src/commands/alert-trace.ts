...
export async function runAlertTrace(deps:CliDeps,trayId:string,raw:{agent?:string;sourceInstance?:string;from?:string}){
 ...
 const root=openRuntimeStore(deps.boxRuntimeRoot,deps.env).root,selector={trayId,...(raw.agent?{agentId:raw.agent}:{}),...(raw.sourceInstance?{sourceInstanceId:raw.sourceInstance}:{})};
