import { openBotHandover, type BotHandoverPort, type HandoverEffect } from "@grokbox/box-runtime/runtime";
...
/** Native formal RPCs only. Notices are sent as the authenticated user when
 * explicitly authorized; this adapter never forges an old Bot sender. */
export function createGatewayBotHandover(deps: CliDeps, scopeId: string, timeoutMs: number,
  authorized?: (request: BotWorkflowRequest) => Promise<boolean>) {
  const gateway = new GatewayClient({ ...deps, transport: "local" }); let generation: string | undefined;
  const check = <A>(response: { result: A; discovery: Discovery }): A => {
  ...
  const rpc = async (method: Parameters<GatewayClient["rpc"]>[0], body: Record<string, unknown>, write = false) => check(await gateway.rpc(method, body,
    { timeoutMs, maxResponseBytes: 512 * 1024, ...(write ? { write: true, singleAttempt: true, unknownOutcomeCode: "operation_outcome_unknown" as const } : {}) }));
  const roster = async () => {
  ...
  const routines = async (id: string) => {
    ...
    const catalog = projectNativeRoutines(id, result);
    return { raw: result as any[], catalog };
  };
  ...
  const noticeRecorded = async (request: BotWorkflowRequest, item: HandoverPlanItem) => {
    const entries = (await tail(String(item.input.destination))).entries;
    const text = notice(request,item);
    return entries.some((e: any) => e?.kind === "message" && e.role === "user" && e.content === text);
  };
  const port: BotHandoverPort = {
    authorize: async request => {
      if (authorized && !await authorized(request)) return false;
    ...
    discover: async (request, targetId, knownIds) => {
      ...
      // Existence of arbitrary cloud jobs/callbacks cannot be disproved from a
      // roster or a bounded history. Keep that dependency as an explicit item.
    ...
    inspect: async (request,item,completed) => {
      ...
      if (item.kind === "routine-enable") {
        ...
        const view = await routines(targetId), row = view.catalog.routines.find(r => r.id === created.routineId);
        const definition = view.raw.find(r => r.id === created.routineId);
    ...
    perform: async (request,item,completed) => {
      ...
      if (item.kind === "old-guidance") {
        const row = await exact(old);
        const marker = `[grokbox-handoff:${request.operationId}] successor=${targetId}`;
        ...
        if(section) await rpc("assignAgentToSidebarSection",{agentId:old,sectionId:section.id},true);
          ...
          // Re-read only the sidebar field immediately before a native update;
          // preserve every other section/member and never write unrelated settings.
        ...
        const members=[...new Set(group.memberIds.map((id:string)=>id===old?targetId:id))];
        await rpc("setGroupMembers",{id:group.id,memberAgentIds:members},true);
  ...
  const program=openBotHandover({durableRoot:deps.boxRuntimeRoot,scopeId,native:port});
  return {program,port,tail,routines,roster};
}
