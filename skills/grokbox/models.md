...
Models use one current schema v3 for both reading and writing. v1/v2 documents, string assignments and the old local `contextWindow` alias are rejected without changing their bytes; the canonical field is `contextWindowTokens`. Pi's own `contextWindow` is accepted only at the explicit Pi catalog adapter. Preserving a model file during general configuration migration does not validate it or make it a current runtime input. Every serving CLI/Host/modeld artifact must match the installed schema before adoption; a new invalid document cannot silently switch to the native model or reinterpret an already-captured TURN. Historical operation lookup remains independent of current model configuration.
