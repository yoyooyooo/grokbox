...
`grokbox bot create --input @file --preview` reviews a declaration containing a persisted `requestId`, `profile.name`, explicit `harness: "box"` and `deferStart: true`. Submit that same file with the returned `--scope-id`, `--expect-revision`, `--accept-non-atomic` and `--confirm`; `bot ownership get <native-id>` separately checks registration after creation. App New Bot is not the operator's custom-model creation path. An unknown create outcome must be inspected, not retried as a second Bot.
