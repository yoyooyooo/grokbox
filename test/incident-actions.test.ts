...
test("incident actions and event watch use real Node HTTP, SQLite and the packaged CLI", async () => {
  const cli = ensurePackedCli(), cache = join(root, "node_modules", ".cache");
  await mkdir(cache, { recursive: true });
  const directory = await mkdtemp(join(cache, "incident-actions-"));
  try {
    const entry = join(directory, "suite.mjs");
    await build({ absWorkingDir: root, entryPoints: ["packages/server/test/incident-actions.node.ts"], outfile: entry,
    ...
    const result = spawnSync("node", ["--test", "--test-reporter=tap", entry], { cwd: directory, encoding: "utf8", timeout: 60_000, maxBuffer: 2 * 1024 * 1024,
    ...
    console.log(JSON.stringify({ suite: "incident-actions-node", tests: Number(result.stdout.match(/# tests (\d+)/)?.[1]), failed: 0, skipped: 0 }));
