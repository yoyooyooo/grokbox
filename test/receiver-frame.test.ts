...
import { nativeReceiverReader } from "../packages/cli/src/gateway-receiver.ts";
...
async function fixture() {
  ...
  const server = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
    ...
    return Response.json({ grokboxRuntimeCapabilities: { version: 1, source: "Host.loaded-runtime-capabilities",
