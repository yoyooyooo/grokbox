...
describe("registry, help, and runtime", () => {
  ...
  test("bundled grokbox skill is version-matched and distinct from core", async () => {
    ...
    expect(product.stdout).toContain("--topic models");
    expect(product.stdout).toContain("[services](services.md)");
    expect(product.stdout).not.toContain("grokbox host start");
    expect(product.stdout).not.toContain("Leaf commands");
    const services = await captureCli(["skills", "get", "grokbox", "--topic", "services"], { skillsDir, discoveryPath: "/dev/null" });
    expect(services.code).toBe(0);
    expect(services.stdout).toContain("grokbox host start");
    ...
    expect(full.stdout).toContain("# Adopt — Host recovery");
