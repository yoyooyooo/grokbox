...
test("console bridge uses real Node HTTP, scoped authentication and persistent domain receipts", async () => {
  const cache = join(root, "node_modules", ".cache");
  await mkdir(cache, { recursive: true });
  const directory = await mkdtemp(join(cache, "web-bridge-"));
  try {
    const entry = join(directory, "suite.mjs");
    await build({ absWorkingDir: root, entryPoints: ["apps/web/test/bridge.node.ts"], outfile: entry,
    ...
    console.log(JSON.stringify({ suite: "web-bridge-node", tests: Number(result.stdout.match(/# tests (\d+)/)?.[1]), failed: 0, skipped: 0 }));
